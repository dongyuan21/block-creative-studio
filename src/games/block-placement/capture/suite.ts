...
import { blockPlacementPacketFromFrame } from '../presentation/legacyPresentationAdapter';

export const CAPTURE_FPS = 30;
export const CAPTURE_RHYTHM_ID = 'human-natural' as const;
export const VIDEO_SIZE = {
  width: blockPlacementCompositionProfile.videoResolution.width,
  height: blockPlacementCompositionProfile.videoResolution.height,
} as const;
...
export const VIDEO_SPECS = [
  { id: 'reference-2d', renderer: 'reference-2d' as const },
...
export function packetForStill(spec: CaptureStillSpec): PresentationPacket {
  return blockPlacementPacketFromFrame(spec.takeId ?? spec.snapshotId ?? spec.id, presentationForSpec(spec));
}

export const blockPlacementCaptureSuite: CaptureSuite<CaptureStillSpec, (typeof VIDEO_SPECS)[number]> = {
  id: 'block-placement.capture.v1',
