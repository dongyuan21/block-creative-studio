...
- Capture/Exporter still render camera/layout from the global `FIXED_SHOT_PROFILE`, not from Plan camera/layout manifests. Reports split `validatedEffectId` / `renderedFxPreset` and `cameraDrivesPixels: false` / `layoutDrivesPixels: false`. `effectDrivesPixels` is true only when the EffectPack `stylePatch.fx` matches the executed `style.fx`.
