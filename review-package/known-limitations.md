...
- React UI drag / Inspector switching is covered only indirectly; the capture harness drives the same Scene/export path without the full App shell. Interactive pick/pointer now maps through the letterboxed composition (`mapClientPointToComposition`); this does not change offline capture frames.
